...
Q_ = simplex(pI,qII,qIII,qIV)

@test is_CSM_tet(P_) == true
@test is_CSM_tet(Q_) == true

sing_ = singularity_detection(P_,Q_)
cv_ref = CommonVertex6D(sing_,SauterSchwab3D._legendre(10,0.0,1.0))
...
list_svert = collect(permutations([pI,qII,qIII,qIV]))
...
print("CommonVertex6D_S test finished")