...
Q_ = simplex(pI,pII,qIV,pIII)

@test is_CSM_tet(P_) == true
@test is_CSM_tet(Q_) == false # <--- intended

sing_ = singularity_detection(P_,Q_)
cf_ref = CommonFace6D(sing_,SauterSchwab3D._legendre(10,0.0,1.0))
...
exact2 = 398633/3628800
exact2_adapted = exact2*(-1) # needed because Q_ is not a CSM-Tetrahedron
err2 = abs(num2-exact2_adapted)/abs(exact2_adapted)
@test err2 < 1.0e-12
...
list_svert = collect(permutations([pI,pII,pIII,qIV]))
...
for tvert in list_tvert
   for svert in list_svert
      ...
      if is_CSM_tet(P) == true && is_CSM_tet(Q) == true
         ...
         cf_s = CommonFace6D_S(sing,(SauterSchwab3D._legendre(6,0.0,1.0),
...
print("CommonFace6D_S test finished")